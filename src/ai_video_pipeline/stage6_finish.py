...
def stitch(attempt: Path, native: bool = False) -> dict:
    attempt = attempt.resolve()
    ...
    contract = load_contract(attempt)
    edit_frame = contract.frame if native else (contract.frame_for_stage("07-edit") or contract.frame)
    output_mode = "native" if native else "delivery"
    transform = contract.delivery_transform if not native else {}
    crop = transform.get("crop") or {}
    if transform.get("allowed") and transform.get("operation") == "center-crop-and-scale":
        picture_filter = (
            f"crop={int(crop['width'])}:{int(crop['height'])}:(iw-{int(crop['width'])})/2:"
            f"(ih-{int(crop['height'])})/2,scale={edit_frame.width}:{edit_frame.height},setsar=1"
        )
    else:
        picture_filter = (
    ...
    normalized = edit_root / "work" / f"normalized-{output_mode}"
    ...
    for index, item in enumerate(ordered, start=1):
        ...
        subprocess.run([
            ...
            f"fps={edit_frame.fps},{picture_filter}",
        ...
        segments.append({"shot_id": item["shot_id"], "candidate_id": item["selected_candidate"],
                         ...
                         "sha256": _sha(target)})
    concat_file = edit_root / "work" / f"concat-{output_mode}.txt"
    concat_file.write_text(
        "".join(f"file '{(attempt / item['normalized']).resolve()}'\n" for item in segments),
        encoding="utf-8",
    )
    suffix = "fast-track-native-rough-cut" if native else "fast-track-rough-cut"
    output = edit_root / "output" / f"{attempt.parent.parent.name}-{attempt.name}-{suffix}.mp4"
    ...
    receipt = {
        ...
        "output_mode": output_mode, "frame_contract": edit_frame.as_dict(),
        "delivery_transform": transform,
    ...
    receipt_name = "receipt-native.json" if native else "receipt.json"
    (edit_root / receipt_name).write_text(json.dumps(receipt, ensure_ascii=False, indent=2), encoding="utf-8")
    return receipt
...
def main() -> int:
    ...
    actions.add_argument("--stitch-native", action="store_true")
    ...
    result = (review(args.attempt) if args.review else
              ...
              append_retries(args.attempt) if args.append_retries else
              stitch(args.attempt, native=args.stitch_native))
